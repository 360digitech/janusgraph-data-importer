...
object EdgeImportIncrement {
  val logger: Logger = LogManager.getLogger("EdgeImportIncrement")

  val usage =
    """
       边的增量导入
    Usage: EdgeImportIncrement [--janusgraph-hosts 10.94.90.121] [--janusgraph-port 8182] E:\360_doc\lolth\call_edge.csv
  """
  ...
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf().setAppName("EdgeImportIncrement")
    ...
    txtFile.map {
      ...
      partitionOfRecords.foreach(record => {
        var edgeCql = ".V().has('name','" + record._1 + "').as('a').V().has('name','" + record._3 + "')" +
          ".coalesce(inE('" + record._2 + "').where(outV().as('a')), addE('" + record._2 + "').from('a'))"

        if (record._2 == "CALL" && record._4.nonEmpty) {
          edgeCql += ".property('mgm'," + record._4.get + ")"
        }
